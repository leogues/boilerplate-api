...
const changed = new Set(changedCsv.split(',').map(f => f.replace(/^src\//, '')));
const entries = results.filter(e => changed.has(e.file_name)).sort((a, b) => b.fta_score - a.fta_score);
